...
class KalshiClient:
    def __init__(self):
        ...
        raw = PRIVATE_KEY_PEM.replace("\\n", "\n").strip().strip('"').strip("'")
        import re
        if "BEGIN" not in raw:
            # Single-line base64-encoded PEM (recommended for Railway)
            b64 = re.sub(r"[^A-Za-z0-9+/=]", "", raw)
            b64 += "=" * (-len(b64) % 4)  # repair padding
            try:
                pem_str = base64.b64decode(b64).decode()
            except Exception as e:
                raise RuntimeError(
                    f"KALSHI_PRIVATE_KEY base64 decode failed "
                    f"(received {len(raw)} chars, expected ~2240): {e}")
            if "BEGIN" not in pem_str or "END" not in pem_str:
                raise RuntimeError(
                    f"KALSHI_PRIVATE_KEY decoded but PEM is incomplete "
                    f"(received {len(raw)} chars, expected ~2240 — "
                    f"value was likely truncated when pasted)")
        else:
            # Raw PEM: rebuild from scratch — find the BEGIN/END block anywhere
            # in the value (tolerates junk, extra dashes, mangled whitespace)
            m = re.search(r"-{3,}\s*BEGIN ([A-Z ]+?)\s*-{3,}(.*?)-{3,}\s*END [A-Z ]+?\s*-{3,}",
                          raw, re.DOTALL)
            if not m:
                raise RuntimeError("KALSHI_PRIVATE_KEY: could not find a PEM block")
            label = m.group(1).strip()
            body = re.sub(r"[^A-Za-z0-9+/=]", "", m.group(2))
            wrapped = "\n".join(body[i:i+64] for i in range(0, len(body), 64))
            pem_str = f"-----BEGIN {label}-----\n{wrapped}\n-----END {label}-----"
        pem = pem_str.encode()
