...
class Maker:
    ...
    def sync_quotes(self, want: dict, exposure: int):
        ...
        # duplicate sweep: the side-misread bug left several resting
        # orders per (ticker, side). Keep the one closest to the wanted
        # price (tie: keep — FIFO priority belongs to the survivor),
        # cancel the rest.
        by_key: dict = {}
        for oid, o in self.live.items():
            by_key.setdefault((o["ticker"], o["side"]), []).append(oid)
        for key, oids in by_key.items():
            if len(oids) < 2:
                continue
            wp = (eff.get(key) or {}).get("price")
            oids.sort(key=lambda i: abs(self.live[i]["price"] - wp)
                      if wp is not None else 0)
            for oid in oids[1:]:
                o = self.live[oid]
                if self.client.cancel_order(oid) == "unavailable":
                    continue
                store.record_quote_event("cancel", oid, o["ticker"],
        ...
        for key, q2 in eff.items():
            ...
            if res.status == "placed":
                ...
                exposure += cost
                if ctx.get("stepped"):
                    note_step(q2["ticker"], q2["side"])   # penny-war radar
