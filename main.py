...
class Maker:
    ...
    def reconcile_from(self, orders: list):
        ...
        for o in orders:
            ...
            fresh[oid] = {
                "ticker": o.get("ticker"), "side": side, "price": px or 0,
    ...
    def poll_fills(self):
        ...
        for f in fills:
            ...
            side = f.get("side", "?")
            px = (f.get("yes_price") if side == "yes" else f.get("no_price")) or 0
            cnt = f.get("count", 0)
            tkr = f.get("ticker", "?")
    ...
    def sync_quotes(self, want: dict, exposure: int):
        books: dict = {}
        # cancel: no longer wanted / fair moved / mispriced
        for oid, o in list(self.live.items()):
            ...
            # v4.7: hold the top of the queue. If a rival has outbid our
            # RESTING order and stepping above them still clears the edge
            # floor, requote — otherwise we sit 1c behind until fair moves.
            outbid = False
            if (C.JOIN_BEST and w is not None
                    and not (stale or moved or mispriced)):
                book = self._book(o["ticker"], books)
                if book:
                    ob = best_bid(book.get(o["side"]) or [])
                    if ob > o["price"]:
                        opp = best_bid(book.get(
                            "no" if o["side"] == "yes" else "yes") or [])
                        implied_ask = 100 - opp if opp else 100
                        cand = ob + 1
                        edge = w["fair"] - cand - maker_fee_cents(cand, 1)
                        if cand < implied_ask and edge >= C.JOIN_MIN_EDGE:
                            outbid = True
            if stale or moved or mispriced or outbid:
                ...
                store.record_quote_event(
                    ...
                    reason="stale" if stale else "moved" if moved
                    else "reprice" if mispriced else "outbid")
                del self.live[oid]
            elif w is not None:
        ...
        # post missing quotes (with orderbook clamp — item C14)
        have = {(o["ticker"], o["side"]) for o in self.live.values()}
        for key, q in want.items():
            if key in have:
                continue
            book = self._book(q["ticker"], books)
