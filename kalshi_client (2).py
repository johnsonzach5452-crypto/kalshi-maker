...
class KalshiClient:
    def __init__(self):
        ...
        # Rebuild the PEM from scratch: extract the base64 body, strip ALL
        # whitespace (Railway may turn newlines into spaces), rewrap at 64.
        import re
        m = re.search(r"-----BEGIN ([A-Z ]+)-----(.*?)-----END \1-----",
                      pem_str, re.DOTALL)
        if m:
            label = m.group(1)          # e.g. "RSA PRIVATE KEY" or "PRIVATE KEY"
